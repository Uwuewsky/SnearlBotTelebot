...
class ModelDB:
    ...
    def by_author(self, chat_id, user_title):
        ...
        res = self.cur.execute("SELECT chat_id, file_id, "
                               ...
                               f"ON Userlist.id = {self.table}.user_id "
                               "WHERE "
                               "(chat_id=:chat_id AND user_title=:query) "
    ...
    def authors_list(self, chat_id):
        ...
        res = self.cur.execute("SELECT DISTINCT user_name,"
                               "user_title, user_nick "
                               "FROM Userlist "
                               f"JOIN {self.table} "
                               f"ON Userlist.id = {self.table}.user_id "
                               "WHERE chat_id=? "
    ...
    def clear_by_chat(self, chat_id):
        """Функция удаления всех данных для чата"""
