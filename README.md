...
Команды `block` и `v` нужно отправить ответом на сообщение. Команду `q` можно использовать через ответ для создания цитаты из одного сообщения. Чтобы цитировать несколько сообщений, их нужно переслать в чат одновременно вписав в поле сообщения команду `/q`.

Цитаты сохраняются в БД, присваиваясь пользователю первого цитированного сообщения. Автора войса можно указать явно вручную через `/v a=1 [Описание]`, где 1 это номер автора в списке `/voicelist`.
